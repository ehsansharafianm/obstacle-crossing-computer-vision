...
- `build_multi_trajectory.py` — **the main tool, `build_multi_trajectory N`**: both feet
  ...
  world frame, **time zeroed at the clap**, the **obstacle reconstructed per frame as
  obstacle1/obstacle2 markers** (so it can be moved between crossings), xlsx
  (markers incl. obstacle1/2 + audio) + plots + audio figure,
