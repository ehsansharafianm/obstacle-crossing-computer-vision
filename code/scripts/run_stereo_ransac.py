...
print("Scanning + PnP cam1..."); P1 = poses("data/cam1_extrinsics.MOV", spec, intr1.camera_matrix, intr1.dist_coeffs)
print("Scanning + PnP cam2..."); P2 = poses("data/cam2_extrinsics.MOV", spec, intr2.camera_matrix, intr2.dist_coeffs)
...
print(f"cam1 holds={len(set(h1))} ({len(P1)} branches), cam2 holds={len(set(h2))} ({len(P2)} branches)")

ROT_TOL, T_TOL = np.radians(4.0), 0.15


def consistent_pairs(R, T):
    Rp = np.einsum("ij,njk->nik", R, R1)
    tp = np.einsum("ij,nj->ni", R, t1) + T
    seen1, seen2, pairs = set(), set(), []
    order = []
...
for a in range(len(R1)):
    for b in range(len(R2)):
        ...
        if len(pr) > len(best_pairs):
            best_pairs = pr
print(f"Largest geometrically-consistent set: {len(best_pairs)} hold-pairs")

if len(best_pairs) < 4:
    raise SystemExit("Still too few consistent pairs.")
...
extr.save("calibration/stereo_extrinsics.npz")
print(f"\n=== STEREO RESULT ===")
print(f"  pairs used = {len(idx)}")
print(f"  stereo RMS reprojection error = {rms:.4f} px")
print(f"  baseline (camera separation)  = {extr.baseline_m():.3f} m")
print(f"  saved -> calibration/stereo_extrinsics.npz "
      f"({'GOOD' if rms < 1.5 else 'still high'})")
